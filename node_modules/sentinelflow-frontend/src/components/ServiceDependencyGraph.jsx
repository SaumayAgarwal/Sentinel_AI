...
export default function ServiceDependencyGraph({ services = [], infraStatus = {}, incidents = [], events = [], onClose }) {
  ...
  // Helper to extract service object from services prop
  const getMicroservice = (name) => {
    ...
    return {
      ...
      port: found?.port || (name === 'user' ? 3001 : name === 'order' ? 3002 : name === 'payment' ? 3003 : 3004),
      latencyMs: found?.lastLatencyMs || 0,
      consecutiveFailures: found?.consecutiveFailures || 0,
      type: 'MICROSERVICE'
    };
  };

  const userSvc = getMicroservice('user');
  const orderSvc = getMicroservice('order');
  const paymentSvc = getMicroservice('payment');
  const inventorySvc = getMicroservice('inventory');

  const microservices = [userSvc, orderSvc, paymentSvc, inventorySvc];
